/**
 * @file
 * Provides UI/UX progressive enhancements on Big Day Boston's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.bdb.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-bdb-custom-color', changedInput.value);
    inputToSync.setAttribute('data-bdb-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-bdb-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-bdb-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes Big Day Boston theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-bdb-custom-color',
      textInput.getAttribute('data-bdb-custom-color'),
    );
  ...
  /**
   * Big Day Boston Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.bdbColorPicker = {
    attach: () => {
      ...
      const colorTextInputs = once(
        'bdb-color-picker',
        '[data-drupal-selector="bdb-color-picker"] input[type="text"]',
      );
